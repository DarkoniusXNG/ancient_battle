...
LinkLuaModifier("modifier_sohei_momentum_knockback", "heroes/sohei/sohei_momentum.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_sohei_momentum_slow", "heroes/sohei/sohei_momentum.lua", LUA_MODIFIER_MOTION_NONE)
  ...
  function modifier_sohei_momentum_passive:OnAttackLanded( event )
    if event.attacker == self:GetParent() and self.attackPrimed == true then
      ...
      if not attacker:HasModifier( "modifier_sohei_flurry_self" ) then
		...
		-- Knock the enemy back if ability is toggled
		if spell:GetToggleState() then
			...
			target:AddNewModifier( attacker, spell, "modifier_sohei_momentum_knockback", {duration = duration, distance = distance, speed = speed, collision_radius = collision_radius} )
		end
...
-- Momentum's knockback modifier
modifier_sohei_momentum_slow = class({})
