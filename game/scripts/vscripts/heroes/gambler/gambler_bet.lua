...
function gambler_bet_on_spell_start(keys)
	...
	local target_enemies = FindUnitsInRadius(target_team, target_location, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
...
function gambler_bet_on_target_death(keys)
	...
	if not target:IsAlive() and target:IsRealHero() and not attacker:IsNull() and (attacker:IsRealHero() or attacker:IsControllableByAnyPlayer()) then
		...
		if attacker_owner ~= caster_owner then
			...
			PlayerResource:ModifyGold(attacker_owner, gold_for_killer, true, DOTA_ModifyGold_Unspecified)
			--attacker:ModifyGold(gold_for_killer, true, DOTA_ModifyGold_Unspecified)
		else
		...
		-- Gold for the caster
		PlayerResource:ModifyGold(caster_owner, gold_for_caster, false, DOTA_ModifyGold_Unspecified)
		--caster:ModifyGold(gold_for_caster, false, DOTA_ModifyGold_Unspecified)
